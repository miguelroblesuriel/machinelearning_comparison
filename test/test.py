...
filename_model = "tutorial_model.model"
modelo_Spec2Vec = gensim.models.Word2Vec.load(filename_model)
...
spectra_dupla = [createSpectrum(ms2_df[ms2_df['scan'] == scan_dupla]['i_norm'].to_numpy(),
                numpy.sort(ms2_df[ms2_df['scan'] == scan_dupla]['mz'].to_numpy())) for scan_dupla in dupla]#creación de los espectros de las duplas
spectra_tripletas = [createSpectrum(ms2_df[ms2_df['scan'] == scan_tripleta]['i_norm'].to_numpy(),
                numpy.sort(ms2_df[ms2_df['scan'] == scan_tripleta]['mz'].to_numpy())) for scan_tripleta in tripletas]#creación de los espectros de las tripletas
